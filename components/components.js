...
class ComponentManager {
	...
	async init() {
		...
		try {
			...
			const { default: ResurgenceHeader } = await import('./header-component.js?v=3.2.0');

			// Load footer component
			const { default: ResurgenceFooter } = await import('./footer-component.js?v=1.7.0');

			// Load intersite navbar
			await import('./intersite-navbar.js?v=1.10.0');
	...
	updatePageContext() {
		...
		} else if (pathname.includes('rp-geopolitique.html')) {
